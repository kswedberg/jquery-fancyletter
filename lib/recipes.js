...
ChiliBook.recipes[ "javascript.js" ] = 
{
    ...
    // , regexp  : { exp: /\/[^\/\\\n]*(?:\\.[^\/\\\n]*)*\/[gim]*/ }
    // , regexp  : { exp: /\/[^<\/\\\n]*(?:\\.[^>\/\\\n]*)*\/[gim]*/ }
    , stepOpen: { exp: /<b class="?step"?>/i, replacement: '<div class="step">' }
    , stepClose: { exp: /<\/b>/i, replacement: '</div>' }
    , amps        : { exp: /&(amp|gt|lt);/, replacement: '<span class="op">&$1;</span>' }
    , emph: {exp: /<\/?(?:strong|em)>/i }
    , regexp  : { exp: /\/[^<\/\\\n]*(?:\\.[^\/\\\n]*)*\/[gim]*/ }
		...
		, jqvar  : { exp: /(\$?[a-zA-Z0-9]+)(?=( |\.))/,
                              replacement: '<span class="var">$1</span>' }
		, method  : { exp: /[a-zA-Z]+/ }
	}
};
